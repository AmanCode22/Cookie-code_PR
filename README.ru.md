...
**Cuckoo Code** — AI-агент для рабочего стола с **нулевой стоимостью токенов**.
...
Веб-чаты прекрасно **думают**, но не могут **действовать** на вашей машине. Cuckoo Code замыкает этот цикл:
...
- Выбор через сетку превью в **Настройки → Cuckoo Code**
...
Ник пользователя в сайдбаре имеет **анимированный радужный градиент** (включено по умолчанию). Отключается в **Настройки → Cuckoo Code → Эффекты**.
...
Cuckoo Code перехватывает его, выполняет в песочнице и возвращает результат AI.
...
Все настройки редактируются через **Настройки → Cuckoo Code**.
...
│   │   ├── settings-tab.js   Вкладка Cuckoo Code в настройках
